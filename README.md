...
민원 처리 법령에는 중요도 1~5 같은 공식 우선순위 점수가 없기 때문에, 민원 처리 절차와 안전 관련 기준의 취지를 참고하여 자체 라벨링 기준을 세웠습니다.
...
- Colab 학습 결과와 웹 테스트 화면을 통해 모델의 동작을 직접 확인한다.
...
중요도 라벨이 붙어 있는 공개 민원 데이터가 따로 없었기 때문에, 이 프로젝트에서는 연구 목적에 맞춰 중요도 기준을 직접 수립하고 그 기준에 따라 가상 라벨 데이터를 생성했습니다.
...
또한 중요도가 라벨링된 공식 민원 데이터가 없었기 때문에, 이 프로젝트의 중요도 라벨은 우리가 직접 수립한 기준에 따라 생성한 가상 라벨입니다. 따라서 개별 민원의 중요도가 항상 정확하다고 볼 수는 없습니다.

그럼에도 이 프로젝트는 의미가 있습니다. 민원 우선 처리 기준을 먼저 정의하고, 그 기준에 맞춰 라벨링한 데이터를 인공지능에게 학습시키면, 새 민원이 들어왔을 때 중요도와 우선순위를 선별하는 보조 시스템을 만들 수 있음을 보여줍니다.

향후 개선 방향은 다음과 같습니다.

- 사람이 직접 검수한 고품질 라벨 추가
- 실제 기관 담당자의 판단 기준을 반영한 라벨링 기준 보완
